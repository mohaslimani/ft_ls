...
/*   Updated: 2019/10/15 20:19:56 by ommadhi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					ft_alloc(DIR *dir, t_node **dr, char *name, int op)
{
	...
	(op & L || op & P) ? ft_print_all(*dr, op, 0) : aff_list(*dr, op);
	closedir(dir);
	return (1);
}
...
int					ft_ifile(char **tab, int op)
{
	...
	while (tab[++i])
	{
		if (lstat(tab[i], &data) != -1 && !(data.st_mode & S_IFDIR) && (k = 1))
		{
	...
	(op & L || op & P) ? ft_print_all(head, op, 1) : aff_list(head, op);
	free_node(&head);
	return (k);
}
...
int					main(int ac, char **av)
{
	...
	ik[3] = ft_ifile(tab, ik[2]);
	while (tab[++ik[0]])
	{
		if (lstat(tab[ik[0]], &data) != -1 && data.st_mode & S_IFDIR)
		{
			(ik[3] == 1 && (ik[3] = 2)) ? ft_putstr("\n") : 0;
